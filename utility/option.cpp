    ...
    YAML::Node& node, GnssCommonOptions& options)
{
  ...
  std::vector<std::string> phase_satellite_excludes;
  if (option_tools::safeGet(node, "phase_satellite_exclude",
                            &phase_satellite_excludes)) {
    options.phase_satellite_exclude.clear();
    for (auto phase_satellite_exclude : phase_satellite_excludes) {
      options.phase_satellite_exclude.push_back(phase_satellite_exclude);
    }
  }
    ...
    YAML::Node& node, AmbiguityResolutionOptions& options)
{
  ...
  LOAD_COMMON(ratio);
  LOAD_COMMON(max_position_jump_ar_horizontal);
  LOAD_COMMON(max_position_jump_ar_vertical);
  LOAD_COMMON(min_num_continuous_nl_fix);
  LOAD_COMMON(use_ppp_nl_fix_hold);
  LOAD_COMMON(max_num_ppp_nl_fix_hold_epoch_gap);
  LOAD_COMMON(max_ppp_nl_fix_hold_residual_cycles);
    ...
    YAML::Node& node, PppEstimatorOptions& options)
{
  ...
  LOAD_COMMON(estimate_velocity);
  LOAD_COMMON(strict_code_bias_mapping);
  LOAD_COMMON(log_code_bias_mapping);
}
...
template <>
void loadOptions<PppImuCameraRrrEstimatorOptions>(
    YAML::Node& node, PppImuCameraRrrEstimatorOptions& options)
{
  LOAD_COMMON(max_keyframes);
  LOAD_COMMON(max_gnss_window_length_minor);
  LOAD_COMMON(min_yaw_std_init_visual);
  // LOAD_COMMON(dpo_constraint_weight);
}
