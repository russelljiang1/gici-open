...
PppImuCameraRrrEstimator::PppImuCameraRrrEstimator(
               const PppImuCameraRrrEstimatorOptions& options,
  ...
  rrr_options_(options), ppp_options_(ppp_options),
...
{
  type_ = EstimatorType::PppImuCameraRrr;
  ...
  spp_options.use_dual_frequency = false;
...
// Add measurement
bool PppImuCameraRrrEstimator::addMeasurement(const EstimatorDataCluster& measurement)
{
...
bool PppImuCameraRrrEstimator::addGnssMeasurementAndState(
    const GnssMeasurement& measurement)
{
  ...
  gnss_common::rearrangePhasesAndCodes(
    curGnss(), true, ppp_options_.strict_code_bias_mapping,
    ppp_options_.log_code_bias_mapping);

  // Correct code bias
  correctCodeBias(curGnss(), false);

  // Correct phase bias (FCB/UPD)
  ...
  // pose and speed and bias block (IMU预积分)
  ...
  addPseudorangeResidualBlocks(curGnss(), states_[index], num_valid_satellite);
  ...
  if (num_satellites_ == 0) {
    ...
    eraseTroposphereParameterBlock(states_[index]);
  ...
  addPhaserangeResidualBlocks(curGnss(), states_[index]);

  // Add doppler residual blocks
  addDopplerResidualBlocks(curGnss(), states_[index], num_valid_satellite,
    false, getImuMeasurementNear(timestamp).angular_velocity);

  //  Add relative errors (时序约束) 
  if (lastGnssState().valid()) {  // maybe invalid here because of long term GNSS absent
    ...
    // troposphere (对流层缓慢变化)
    addRelativeTroposphereResidualBlock(lastGnssState(), states_[index]);
    // ionosphere (电离层缓慢变化)
...
bool PppImuCameraRrrEstimator::addImageMeasurementAndState(
  const FrameBundlePtr& frame_bundle, const SpeedAndBias& speed_and_bias)
{
  ...
  // 输出DPO三元组约束数量
  size_t num_dpo = numDPOConstraints(states_[index]);
  static size_t cumulative_dpo = 0;
  cumulative_dpo += num_dpo;
  LOG(INFO) << "=== DPO Triplet Constraints: " << num_dpo
            << " (cumulative: " << cumulative_dpo << ") ===";
  ...
  /*
  // 写入CSV
  static std::ofstream constraint_csv("po_timing.csv", std::ios::app);
...
bool PppImuCameraRrrEstimator::visualInitialization(const FrameBundlePtr& frame_bundle)
{
  //  store poses 
...
// Solve current graph
bool PppImuCameraRrrEstimator::estimate()
{
  // ===== 开始计时 =====
  ...
  // ===== 输出状态维度 =====
  {
    ...
    size_t full_dim = 0;
    size_t num_blocks = 0;
    for (const auto& id_to_block : graph_->idToParameterBlockMap()) {
      if (!id_to_block.second->fixed()) {
        state_dim += id_to_block.second->minimalDimension();
      }
      full_dim += id_to_block.second->dimension();
      num_blocks++;
    }

    //LOG(INFO) << "=== State Dimension: " << state_dim
    //          << " (blocks: " << num_blocks << ") ===";

    // 保存到文件
    static std::ofstream mem_file("po_state.csv", std::ios::app);
    if (mem_file.is_open()) {
      static bool header_written = false;
      if (!header_written) {
        mem_file << "timestamp,state_dimension,full_dimension,num_blocks," << std::endl;
        header_written = true;
      }
      mem_file << std::fixed << std::setprecision(6)
               << getTimestamp() << ","
               << state_dim << "," << full_dim << "," << num_blocks  << std::endl;
    }
  }
...
void PppImuCameraRrrEstimator::setInitializationResult(
...
// Marginalization
bool PppImuCameraRrrEstimator::marginalization(const IdType& type)
{
  if (type == IdType::cPose) {
    return frameMarginalization();
  }
...
bool PppImuCameraRrrEstimator::frameMarginalization()
{
  ...
  if (states_[latest_state_index_].is_keyframe &&
    ...
    for (auto it = states_.begin(); it != states_.end();) {
      ...
      // GNSS state that not yet been marginalized by GNSS steps.
        ...
        auto it_ionosphere = ionosphereStateAt(state.timestamp);

        if (it_ambiguity != ambiguity_states_.end() &&
            it_ionosphere != ionosphere_states_.end()) {
          addAmbiguityMarginBlocksWithResiduals(*it_ambiguity);
          addIonosphereMarginBlocksWithResiduals(*it_ionosphere);
        }
        else {
        ...
        addTroposphereMarginBlockWithResiduals(state);
        addFrequencyMarginBlocksWithResiduals(state);
        addGnssResidualMarginBlocks(state);

        // erase ambiguity, ionosphere state and GNSS measurements
        if (it_ambiguity != ambiguity_states_.end()) {
          ambiguity_states_.erase(it_ambiguity);
        }
        if (it_ionosphere != ionosphere_states_.end()) {
          ionosphere_states_.erase(it_ionosphere);
        }
...
bool PppImuCameraRrrEstimator::gnssMarginalization()
{
  // Check if we need marginalization
  if (isFirstEpoch()) return true;
...
void PppImuCameraRrrEstimator::sparsifyGnssStates()
{
  ...
  for (int i = 0; i < states_.size(); i++) {
    ...
    if (i + 1 < states_.size()) for (int j = i + 1; j < states_.size(); j++) {
      if (states_[j].id.type() != IdType::gPose) break;
      num_neighbors[idx]++;
    }
  ...
  for (int i = 0; i < states_.size(); i++) {
    ...
    eraseTroposphereParameterBlock(state);

    // we may failed to find corresponding ambiguity/ionosphere state because the GNSS state
    // can be loosely coupled state during initialization.
    if (it_ambiguity != ambiguity_states_.end() &&
        it_ionosphere != ionosphere_states_.end()) {
      eraseAmbiguityParameterBlocks(*it_ambiguity);
      ambiguity_states_.erase(it_ambiguity);
      eraseIonosphereParameterBlocks(*it_ionosphere);
      ionosphere_states_.erase(it_ionosphere);
    }
    else {
...
bool PppImuCameraRrrEstimator::estimateAmbiguityCovariance(
...
bool PppImuCameraRrrEstimator::buildDPOConstraints(
...
bool PppImuCameraRrrEstimator::addDPOConstraint(
...
void PppImuCameraRrrEstimator::updateFeatureTracksWithNewFrame(
...
void PppImuCameraRrrEstimator::eraseDPOConstraints(const State& state) {
...
size_t PppImuCameraRrrEstimator::numDPOConstraints(const State& state) {
...
void PppImuCameraRrrEstimator::rejectDPOConstraintOutlier(const State& state) {
