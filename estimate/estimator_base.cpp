...
bool EstimatorBase::applyMarginalization()
{
  ...
  else if (marginalization_error_->num_residuals() > 0) {
  // Even if no parameter blocks are marginalized, we still need to update
  // error computation if residuals were added
  marginalization_error_->updateErrorComputation();
  }         
