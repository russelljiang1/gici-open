...
{
  if (data_buf_.size() == 0 && !valid()) return false;

  // Load from file if data used-up
  if (data_buf_.size() == 0 && !load()) return false;
...
{
  if (!fd_ || !formator_ || !buf_) return false;
  ...
  while (!feof(fd_))
  {
    ...
    if (nread == 0) {
      if (ferror(fd_)) {
        LOG(ERROR) << "Failed to read file " << path_ << " for " << tag_;
      }
      break;
    }
  ...
  if (!has_new_data && feof(fd_)) {
    std::vector<std::shared_ptr<DataCluster>> dataset;
    int nobs = formator_->decode(nullptr, 0, dataset);
    for (int iobs = 0; iobs < nobs; iobs++) {
      std::shared_ptr<DataCluster>& data = dataset.at(iobs);
      double timestamp = burst_load_ ? 0.0 : getTimestamp(data);
      data_buf_.push_back(std::make_pair(timestamp, data));
