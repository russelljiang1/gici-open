                ...
                const std::shared_ptr<DataCluster::GNSS>& gnss)
{
  ...
  for (auto it : gnss->types) {
    ...
    if (it == GnssDataType::PhaseCenter) {
      ...
      // Path B: take ownership of the receiver-antenna PCV table so the
      // estimator can apply receiver-side PCO/PCV later.
      if (gnss->receiver_pcvs && gnss->receiver_pcvs->n > 0) {
        if (gnss_local_->receiver_pcvs == nullptr) {
          gnss_local_->receiver_pcvs = (pcvs_t *)malloc(sizeof(pcvs_t));
          memset(gnss_local_->receiver_pcvs, 0, sizeof(pcvs_t));
        }
        if (gnss_local_->receiver_pcvs->pcv) {
          ::free(gnss_local_->receiver_pcvs->pcv);
        }
        int n = gnss->receiver_pcvs->n;
        gnss_local_->receiver_pcvs->pcv = (pcv_t *)malloc(sizeof(pcv_t) * n);
        gnss_local_->receiver_pcvs->n = n;
        gnss_local_->receiver_pcvs->nmax = n;
        memcpy(gnss_local_->receiver_pcvs->pcv,
               gnss->receiver_pcvs->pcv, sizeof(pcv_t) * n);
        LOG(INFO) << "PhaseCenter: loaded " << n
                  << " receiver-antenna entries from ATX.";
      } else {
        LOG(WARNING) << "PhaseCenter: ATX file produced 0 receiver-antenna "
                        "entries. Receiver-side PCO/PCV correction unavailable.";
      }
  ...
  // Check if we should wait for more ephemeris. For mixed-system post-processing,
  // some observed satellites may never have usable precise/broadcast ephemeris in
  // the provided products. If we already have enough valid satellites to solve,
  // proceed instead of waiting forever on the invalid-ratio threshold.
  // Keep this threshold aligned with the downstream GNSS solver minimum.
  // Requiring more here can stall offline playback on epochs that are already
  // solvable, especially when many observed satellites have no matching orbit/clock.
  const int min_valid_ephemeris_to_process = 5;
  ...
  if (invalid_ephemeris_ratio > 0.2 &&
      num_valid_ephemeris < min_valid_ephemeris_to_process) {
  ...
  // Set phase center
  // Path B: bind the receiver antenna PCV (lookup by RINEX `ANT # / TYPE`)
  // on the first rover epoch that arrives. After the call the binding
  // sticks for the rest of the run, so subsequent epochs reuse it for free.
  if (phase_center_local_ != nullptr &&
      !phase_center_local_->hasReceiverPcv() &&
      gnss_local_->receiver_pcvs != nullptr &&
      role_out == GnssRole::Rover) {
    // searchpcv splits the input string by whitespace into up to 2 tokens
    // and matches them as substrings of pcv->type, falling back to a
    // no-radome match if the radome variant is missing. Passing the raw
    // antdes alone (e.g. "CHCI85") is sufficient.
    // Try the per-call cluster first (RINEX header info travels with
    // gnss_data[0] which is also where the first observation epoch lands),
    // then fall back to gnss_local_->antenna in case it is set there.
    std::string key;
    if (gnss->antenna != nullptr) key = gnss->antenna->antdes;
    if (key.find_first_not_of(' ') == std::string::npos &&
        gnss_local_->antenna != nullptr) {
      key = gnss_local_->antenna->antdes;
    }
    // Trim trailing spaces (RINEX field is fixed-width).
    while (!key.empty() && key.back() == ' ') key.pop_back();
    if (key.empty()) {
      LOG(WARNING)
        << "PhaseCenter: cannot bind receiver antenna - antdes is empty in "
           "both the per-call gnss cluster and gnss_local_. "
           "Receiver-side PCO/PCV correction will be skipped.";
    } else {
      phase_center_local_->setReceiverAntenna(
          key,
          gnss_local_->receiver_pcvs,
          gnss->observation->data[0].time);
    }
  }
...
void GnssDataIntegration::updateCodeBias()
{
  ...
  LOG(INFO) << "GnssDataIntegration: Applied code biases from SSR. count=" << num_set;
}
...
void GnssDataIntegration::updatePhaseBias()
{
  nav_t *nav = gnss_local_->ephemeris;
  int num_set = 0;
  std::map<char, int> system_counts;
  for (int i = 0; i < MAXSAT; i++) {
  ...
  oss << "GnssDataIntegration: Applied phase biases from SSR. count=" << num_set;
  if (!system_counts.empty()) {
    oss << ", by_system=";
    bool first = true;
    for (const auto& item : system_counts) {
      if (!first) oss << ",";
      first = false;
