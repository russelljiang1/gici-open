...
public:
  ...
  struct GNSS {
    ...
    obs_t *observation = nullptr;
    nav_t *ephemeris = nullptr;
    sta_t *antenna = nullptr;
    // Path B: full receiver-antenna PCV table parsed from ATX
    // (entries with pcv_t::sat == 0). Populated only by AtxFileFormator;
    // nullptr otherwise. Lookup by antenna type string at runtime.
    pcvs_t *receiver_pcvs = nullptr;
    bool released = false;
  };
...
protected:
  std::vector<char> system_exclude_;
  rnxctr_t rnx_;
...
public:
  ...
  using OsbContainer =
    std::map<char, std::map<int, std::map<std::string, OsbData>>>;
...
protected:
  ...
  OsbContainer code_osbs_;
  OsbContainer phase_osbs_;
};
