...
void FilesReading::run()
{
  ...
  bool finished_all_readers = false;
  while (!quit_thread_ && SpinControl::ok()) {
    ...
    quit_thread_ = true;
    finished_all_readers = true;
    for (size_t i = 0; i < tags_.size(); i++) {
      std::shared_ptr<FileReaderBase>& reader = file_readers_[i];
      if (!reader->done()) {
        quit_thread_ = false;
        finished_all_readers = false;
      }
    }
  }

  if (finished_all_readers && SpinControl::ok()) {
    LOG(INFO) << "FilesReading: Post-file replay completed. Shutting down.";
    SpinControl::kill();
  }
}
