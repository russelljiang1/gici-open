  ...
  sta_t *sta, std::shared_ptr<DataCluster::GNSS>& gnss_data)
{
  ...
  // Copy antenna/receiver identification strings so downstream code
  // (e.g. PhaseCenter receiver-PCV lookup) can use them.
  memcpy(gnss_data->antenna->name,    sta->name,    MAXANT);
  memcpy(gnss_data->antenna->marker,  sta->marker,  MAXANT);
  memcpy(gnss_data->antenna->antdes,  sta->antdes,  MAXANT);
  memcpy(gnss_data->antenna->antsno,  sta->antsno,  MAXANT);
  memcpy(gnss_data->antenna->rectype, sta->rectype, MAXANT);
  memcpy(gnss_data->antenna->recver,  sta->recver,  MAXANT);
  memcpy(gnss_data->antenna->recsno,  sta->recsno,  MAXANT);
  gnss_data->antenna->antsetup = sta->antsetup;
  gnss_data->antenna->itrf     = sta->itrf;
  gnss_data->antenna->deltype  = sta->deltype;
  gnss_data->antenna->hgt      = sta->hgt;
  for (int i = 0; i < 3; i++) {
    gnss_data->antenna->del[i] = sta->del[i];
  }
  for (int i = 0; i < 3; i++) {
