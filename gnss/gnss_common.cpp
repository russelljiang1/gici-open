...
void rearrangePhasesAndCodes(GnssMeasurement& measurement,
                             bool accept_coarse,
                             bool strict_code_bias_mapping,
                             bool log_code_bias_mapping)
{
  CodeBiasPtr code_bias = measurement.code_bias;
  size_t num_already_default = 0;
  size_t num_exact = 0;
  size_t num_coarse = 0;
  size_t num_dropped = 0;
  std::map<char, size_t> exact_by_system;
  std::map<char, size_t> coarse_by_system;
  std::map<char, size_t> dropped_by_system;
  std::vector<std::string> exact_samples;
  std::vector<std::string> coarse_samples;
  std::vector<std::string> dropped_samples;

  auto codeString = [](char system, int code_type) {
    return gnss_common::codeTypeToRinexType(system, code_type);
  };

  auto pushSample = [&](std::vector<std::string>& samples,
                        const std::string& prn,
                        char system,
                        int code,
                        int default_code,
    ...
    oss << prn << ":" << codeString(system, code)
        << "->" << codeString(system, default_code)
        << "(" << tag << ")";
  ...
  for (auto& sat : measurement.satellites) {
    ...
    std::unordered_map<int, int> arranged_priority;
...
#undef MAP
      bool can_add = true;
      int mapping_priority = -1;
      // do not need to arrange
      if (default_code == code) {
        arranged_observation = *it;
        mapping_priority = 3;
        num_already_default++;
      }
      // arrange to default code
        ...
        if (default_code == 0) {
          can_add = false;
          num_dropped++;
          dropped_by_system[system]++;
          if (dropped_samples.size() < 8) {
            std::ostringstream oss;
            oss << prn << ":" << codeString(system, code)
                << "->NONE(no_default_code)";
            dropped_samples.push_back(oss.str());
          }
        }
        else {
          double bias_exact = code_bias->getCodeBias(prn, code, false);
          double default_bias_exact = code_bias->getCodeBias(prn, default_code, false);
          if (bias_exact != 0.0 && default_bias_exact != 0.0) {
            observation.pseudorange += bias_exact - default_bias_exact;
            arranged_observation = std::make_pair(default_code, observation);
            mapping_priority = 2;
            num_exact++;
            exact_by_system[system]++;
            pushSample(exact_samples, prn, system, code, default_code, "exact");
          }
          else if (strict_code_bias_mapping) {
            can_add = false;
            num_dropped++;
            dropped_by_system[system]++;
            pushSample(dropped_samples, prn, system, code, default_code, "missing_exact");
          }
          else if (accept_coarse) {
            double bias_coarse = code_bias->getCodeBias(prn, code, true);
            double default_bias_coarse = code_bias->getCodeBias(prn, default_code, true);
            if (bias_coarse != 0.0 && default_bias_coarse != 0.0) {
              observation.pseudorange += bias_coarse - default_bias_coarse;
              arranged_observation = std::make_pair(default_code, observation);
              mapping_priority = 1;
              num_coarse++;
              coarse_by_system[system]++;
              pushSample(coarse_samples, prn, system, code, default_code, "coarse_bias");
            }
            else {
              arranged_observation = std::make_pair(default_code, observation);
              mapping_priority = 0;
              num_coarse++;
              coarse_by_system[system]++;
              pushSample(coarse_samples, prn, system, code, default_code, "zero_assumption");
            }
          }
          else {
            can_add = false;
            num_dropped++;
            dropped_by_system[system]++;
            pushSample(dropped_samples, prn, system, code, default_code, "missing_bias");
          }
        }
      }
      // add to observations
      if (can_add) {
        if (arranged_observations.find(default_code) == arranged_observations.end() ||
            arranged_priority[default_code] < mapping_priority) {
          arranged_priority[default_code] = mapping_priority;
          arranged_observations.insert(arranged_observation);
          arranged_observations[default_code] = arranged_observation.second;
        }
      }
    }
  ...
  if (log_code_bias_mapping && (num_exact > 0 || num_coarse > 0 || num_dropped > 0)) {
    auto appendSystemCounts = [](std::ostringstream& oss,
                                 const std::map<char, size_t>& counts) {
      bool first = true;
    ...
    auto appendSamples = [](std::ostringstream& oss,
    ...
    std::ostringstream oss;
    oss << "Code bias mapping: already_default=" << num_already_default
        << ", exact=" << num_exact
        << ", coarse_fallback=" << num_coarse
        << ", dropped=" << num_dropped
        << ", strict=" << strict_code_bias_mapping;
    if (!exact_by_system.empty()) {
      oss << ", exact_by_system=";
      appendSystemCounts(oss, exact_by_system);
    }
    if (!coarse_by_system.empty()) {
      oss << ", coarse_by_system=";
      appendSystemCounts(oss, coarse_by_system);
    }
    if (!dropped_by_system.empty()) {
      oss << ", dropped_by_system=";
      appendSystemCounts(oss, dropped_by_system);
    }
    if (!exact_samples.empty()) {
      oss << ", exact_samples=";
      appendSamples(oss, exact_samples);
    }
    if (!coarse_samples.empty()) {
      oss << ", coarse_samples=";
      appendSamples(oss, coarse_samples);
    }
    if (!dropped_samples.empty()) {
      oss << ", dropped_samples=";
      appendSamples(oss, dropped_samples);
    }
    LOG(INFO) << oss.str();
  }
}
