  ...
  AmbiguityState& state)
{
  ...
  for (auto& sat : measurement.satellites) 
  {
    ...
    if (!gnss_common::useSystem(gnss_base_options_.common, system) || 
        !gnss_common::useSatellite(gnss_base_options_.common, satellite.prn)) continue;
    if (!gnss_common::useSatellite(
          gnss_base_options_.common.phase_satellite_exclude, satellite.prn)) continue;
    if (satellite.ionosphere_type == IonoType::None) continue;
...
void GnssEstimatorBase::correctPhaseBias(GnssMeasurement& measurement)
{
  ...
  std::map<char, size_t> corrected_by_system;
  std::map<char, size_t> missing_by_system;
  ...
  for (auto& sat : measurement.satellites) {
    ...
    for (auto& obs : satellite.observations) {
      ...
      if (bias == 0.0) {
        ...
        num_missing++;
        missing_by_system[system]++;
        if (missing_samples.size() < 12) {
          std::ostringstream oss;
          oss << prn << ":pid=" << phase_id << ":code=" << code;
          missing_samples.push_back(oss.str());
        }
        observation.phaserange = 0.0;
      }
      else {
        num_corrected++;
        corrected_by_system[system]++;
  ...
  if (num_considered > 0) {
    ...
    std::ostringstream oss;
    oss << "PPP Phase OSB: considered=" << num_considered
        << ", corrected=" << num_corrected
        << ", missing=" << num_missing
        << ", zeroed=" << num_missing;
    if (!corrected_by_system.empty()) {
      oss << ", corrected_by_system=";
      appendSystemCounts(oss, corrected_by_system);
    }
    if (!missing_by_system.empty()) {
      oss << ", missing_by_system=";
      appendSystemCounts(oss, missing_by_system);
      oss << ", zeroed_by_system=";
      appendSystemCounts(oss, missing_by_system);
    }
    if (!missing_samples.empty()) {
      oss << ", missing_samples=";
      appendSamples(oss, missing_samples);
    }
    LOG(INFO) << oss.str();
  }
}
