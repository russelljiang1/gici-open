    ...
    const GnssMeasurement& measurement)
{
  ...
  if (curGnss().phase_bias->valid()) {
    // 保存修正前的观测值
    std::map<std::string, std::map<int, double>> phaserange_before;
    for (const auto& sat : curGnss().satellites) {
      for (const auto& obs : sat.second.observations) {
        if (obs.second.phaserange != 0.0) {
          phaserange_before[sat.first][obs.first] = obs.second.phaserange;
        }
      }
    }
    
    LOG(INFO) << "=== Phase Bias Correction Start ===";
    LOG(INFO) << "Total satellites with observations: " << phaserange_before.size();
    ...
    for (const auto& sat : curGnss().satellites) {
      for (const auto& obs : sat.second.observations) {
        if (obs.second.phaserange == 0.0) continue;
        
        auto it_sat = phaserange_before.find(sat.first);
        if (it_sat == phaserange_before.end()) continue;
        
        auto it_obs = it_sat->second.find(obs.first);
        if (it_obs == it_sat->second.end()) continue;
        
        double before = it_obs->second;
        double after = obs.second.phaserange;
        double change = after - before;
        
        if (std::abs(change) > 1e-6) {  // 有显著变化
          corrected_count++;
          max_change = std::max(max_change, std::abs(change));
          LOG(INFO) << "  Corrected: " << sat.first 
                    << " code=" << obs.first
                    << " before=" << std::fixed << std::setprecision(3) << before
                    << " after=" << after
                    << " change=" << change;
        }
      }
    ...
    LOG(INFO) << "=== Phase Bias Correction End ===";
    LOG(INFO) << "Corrected " << corrected_count << " observations, "
              << "max change: " << std::scientific << std::setprecision(3) 
              << max_change << " meters";
  }
  else {
    LOG(WARNING) << "Phase bias is NOT valid! Skipping correction.";
  }
