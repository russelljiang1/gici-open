    ...
    double** jacobians_minimal) const
{
  ...
  // Receiver-side antenna PCO + PCV correction (path B). Returns 0 when
  // no receiver PCV is bound. The convention matches rtklib's antmodel():
  // the value should be ADDED to the modeled range (or equivalently
  // subtracted from the observed range).
  double receiver_antenna_delay = 0.0;
  if (measurement_.phase_center) {
    receiver_antenna_delay = measurement_.phase_center->receiverAntennaDelay(
        gnss_common::systemConvert(satellite_.getSystem()),
        static_cast<uint8_t>(observation_.raw_code),
        azimuth, elevation);
  }
