...
struct AmbiguityResolutionOptions {
  ...
  // Reject PPP AR if one fixed update moves the current position too far (m)
  double max_position_jump_ar_horizontal = 0.15;
  double max_position_jump_ar_vertical = 0.25;

  // Require the same NL integer on the same BSD pair for consecutive epochs
  int min_num_continuous_nl_fix = 3;

  // Keep accepted PPP NL integers through short single-epoch AR dropouts.
  bool use_ppp_nl_fix_hold = true;
  int max_num_ppp_nl_fix_hold_epoch_gap = 3;
  double max_ppp_nl_fix_hold_residual_cycles = 0.5;
};
...
public:
  ...
  struct IntegerStability {
    long long integer = 0;
    int count = 0;
    uint64_t last_epoch_id = 0;
  };
  ...
  struct HeldNlFix {
    ...
    int num_accepted = 0;
    int num_failed = 0;
    uint64_t last_epoch_sequence = 0;
  };
...
private:
  ...
  // Refresh local ambiguity values from graph parameter blocks
  void refreshCurrentAmbiguityValuesFromGraph();

  // PPP NL fixed-hold helpers
  std::string pppNlLaneKey(const LanePair& lane_pair);
  bool pppNlLaneHasSlip(
    const LanePair& lane_pair, const GnssMeasurement& measurements);
  bool validatePppFixedSolution(
    const BackendId& epoch_id, const std::string& source);
  void updateHeldPppNlFixesFromAccepted();
  bool tryApplyHeldPppNlFixes(
...
private:
  ...
  // NL integer stability history
  std::unordered_map<std::string, IntegerStability> nl_integer_stability_;
  uint64_t current_epoch_id_for_stability_ = 0;

  // Accepted PPP NL fixed integers for short hold through AR dropouts
  std::unordered_map<std::string, HeldNlFix> held_ppp_nl_fixes_;
  uint64_t ppp_ar_epoch_sequence_ = 0;
  uint64_t current_ppp_ar_epoch_sequence_ = 0;
